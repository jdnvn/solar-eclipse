...
coords = [[convert_coords(row[1]), convert_coords(row[2]), convert_coords(row[3])] for row in table]
...
eighty_path = [penumbra_coords(0.80, coords[0], coords[1], azm[idx], path_widths[idx]) for idx, coords in enumerate(list(northern_limit_path))]